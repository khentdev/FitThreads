<template>
  ...
  >
    <nav
      class="flex flex-row gap-6 justify-between items-center w-full md:flex-col md:mt-auto md:mb-auto"
    >
      <template
        v-for="link in navigationLinks.filter((l) => !l.toBottom)"
        :key="link.name"
      >
        <RouterLink
          :to="{ name: link.name, params: { username: link.param?.value } }"
          custom
        ...
        >
          <a
            ...
            :class="[
              isExactActive || link.isOwnProfile?.value
                ? 'text-text-default'
                : 'text-text-muted',
              link.activeBg ? 'bg-surface-elevated' : '',
            ]"
            :aria-label="link.label"
            @click.prevent="handleNavigate(link, navigate)"
          >
            <component
              :is="link.icon"
              class="transition-all duration-200 size-7"
              :class="[
                isExactActive || link.isOwnProfile?.value
                  ? 'text-text-default stroke-3'
                  : 'text-text-muted group-hover:text-text-default group-hover:stroke-3',
              ]"
            />
          </a>
        </RouterLink>
      ...
      >
        ...
        >
          <a
            ...
            :class="[isExactActive ? 'text-text-default' : 'text-text-muted']"
            :aria-label="link.label"
            @click.prevent="handleNavigate(link, navigate)"
          >
            <component
...
const { retryFetchProfile } = useProfileStore();

const isOwnProfile = ref(false);
watch(
  [() => authStore.getUsername, () => route.params["username"]],
...
const handleNavigate = (link: NavigationLink, navigate: () => void) => {
  ...
  if (
    link.name === "profile" &&
    ["profile-thoughts", "profile-favorites"].includes(route.name as string)
  ) {
    retryFetchProfile();
  }
