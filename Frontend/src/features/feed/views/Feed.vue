<template>
    <FeedViewLayout title="Feed">
        ...
        tag="div" class="flex flex-col relative w-full">
            <RefetchLoading v-show="!query.isError.value && query.isRefetching.value" key="refetch" />
            <PostSkeleton v-if="query.isPending.value" key="skeleton" />
            <ErrorRetry v-else-if="query.isError.value" key="error" :message="query.errorMessage.value"
                :is-retrying="query.isRefetching.value" :retryFn="handleTryAgain" />
                
            <EmptyState v-else-if="allPosts.length === 0" key="empty" title="No posts yet"
                message="Be the first to share your fitness thoughts." />
                
            <div v-else key="posts" class="flex flex-col px-0">
                <div v-for="(post,i) in allPosts" :key="post.id"
