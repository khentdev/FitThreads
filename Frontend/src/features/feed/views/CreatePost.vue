<template>
  <FeedViewLayout title="Create Post" :showMenu="false">
    <div class="flex flex-col flex-1 w-full min-h-0 md:px-0">
      <div
        class="flex overflow-hidden flex-col flex-1 p-4 md:rounded-xl border-none bg-surface-app"
      >
        ...
        <div class="flex flex-col flex-1 mb-4 min-h-0">
          <textarea
            v-model="content"
            placeholder="Share something authentic. No fluff, just fitness."
            id="contentInput"
            @input="onContentInput"
            class="flex-1 pl-0 w-full text-base leading-relaxed bg-transparent border-l-2 border-transparent transition-all duration-200 outline-none resize-none md:text-lg text-text-default placeholder:text-text-muted/30 focus:border-solid-primary focus:pl-3 focus:placeholder:text-text-muted/50"
          ></textarea>
        </div>
        <div class="flex-none pt-4 border-t border-border-muted/30">
          <div
            class="flex items-center mb-4 text-xs font-medium text-text-muted/60"
          >
          ...
          <div class="space-y-4">
            <div class="relative group">
              <div
                :class="[
                  tags.length >= 5
                    ? 'bg-surface-elevated/50 cursor-not-allowed opacity-60'
                    : '',
                ]"
            ...
            <div
              v-if="tags.length > 0"
              class="flex flex-wrap gap-2 duration-200 break-all"
            >
              <span
                v-for="tag in tags"
                :key="tag"
...
const handleCreatePost = async () => {
  ...
  try {
    const { success } = await feedStore.createPost({
      title: title.value,
      content: content.value,
      postTags: tags.value,
    });
