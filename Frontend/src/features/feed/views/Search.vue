  ...
  <FeedViewLayout title="Search">
    <div class="flex flex-col">
      <div class="p-5 pb-0">
        ...
        >
          <Search
            class="absolute left-5 pointer-events-none size-5 text-border-strong"
          />
          <input
            type="search"
            enterkeyhint="search"
      ...
      <template v-if="hasSearched">
        <div class="flex gap-4 mt-4 w-full">
          ...
          >
            <span
              class="transition-transform duration-150 ease-in-out transform group-active:scale-90"
        ...
        <div
          ...
          v-if="activeFilter === 'top' || activeFilter === 'recent'"
        >
          <TransitionGroup
            :key="activeFilter + searchQuery"
            name="list"
            move-class="transition-all duration-200 ease"
            ...
            leave-active-class="absolute w-full transition-all duration-200 ease"
            tag="div"
          ...
          >
            <RefetchLoading
              key="refetch"
              v-show="
                !activeSearchFeed.isError.value &&
                activeSearchFeed.isRefetching.value
              "
            />
            <PostSkeleton
              v-if="activeSearchFeed.isLoading.value"
              key="skeleton"
            />
            <ErrorRetry
              v-else-if="activeSearchFeed.isError.value"
              :message="activeSearchFeed.errorMessage.value"
              :is-retrying="activeSearchFeed.isRefetching.value"
              :retryFn="handleTryAgainFeed"
              key="error"
            />
            <EmptyState
              v-else-if="feedResults.length === 0"
            ...
            <template v-else>
              ...
              >
                ...
                <h2
                  class="mb-2 text-lg font-bold text-text-default wrap-break-word"
                >
                  {{ post.title }}
                </h2>
                <div class="mb-4">
                  <p
                    class="text-[15px] leading-relaxed text-text-default wrap-break-word"
                  >
                    {{ post.content }}
                  </p>
                </div>
                ...
                <div class="flex gap-6">
                  <button
                    @click="handleToggleLike({ postId: post.id })"
                  ...
                  >
                    <Flame
                      class="size-5 like-icon group-active:stroke-red-500 group-active:fill-red-500"
                      :class="{
                        'fill-red-500 stroke-red-500': post.hasLikedByUser,
                      }"
                    />
                    <span
                      class="text-sm font-medium group-active:text-red-500"
                      :class="{ 'text-red-500': post.hasLikedByUser }"
                      >{{ post._count.likes }}</span
                    >
                  </button>
                  <button
                    @click="handleToggleFavorite({ postId: post.id })"
                    ...
                    <Bookmark
                      ...
                      :class="{
                        'fill-orange-500 stroke-orange-500':
                          post.hasFavoritedByUser,
                      }"
                    />
                    <span
                      class="text-sm font-medium group-active:text-orange-500"
                      :class="{ 'text-orange-500': post.hasFavoritedByUser }"
                      >{{ post._count.favorites }}</span
                    >
                  </button>
                </div>
              </div>
            ...
            <template
              v-if="!activeSearchFeed.isError.value"
              key="templateNoError"
            >
              <EmptyState
                key="noPageAndHasResultsState"
                v-if="
                  !activeSearchFeed.hasNextPage.value && feedResults.length > 0
                "
              ...
              <button
                key="hasNextPage"
                v-if="activeSearchFeed.hasNextPage.value"
                @click="handleLoadMoreFeed"
                :disabled="
                  !activeSearchFeed.hasNextPage.value ||
                  activeSearchFeed.isFetchingNextPage.value
                "
        ...
        <div class="flex flex-col mt-4" v-else-if="activeFilter === 'profile'">
          <TransitionGroup
            :key="activeFilter + searchQuery"
            name="list"
            move-class="transition-all duration-200 ease"
          ...
          >
            <RefetchLoading
              v-show="
                !searchProfiles.isError.value &&
                searchProfiles.isRefetching.value
              "
              key="refetch"
            />
            <ProfileSearchSkeleton
              v-if="searchProfiles.isLoading.value"
              key="skeleton"
              :count="10"
            />
            <ErrorRetry
              v-else-if="searchProfiles.isError.value"
              key="error"
              :message="searchProfiles.errorMessage.value"
              :is-retrying="searchProfiles.isRefetching.value"
              :retryFn="handleTryAgainProfile"
            />
            <EmptyState
              v-else-if="profileResults.length === 0"
              key="empty"
            ...
            <template v-else>
              <div
                v-for="profile in profileResults"
                ...
                "
                class="flex flex-col justify-center px-5 py-4 border-b bg-surface-app cursor-pointer min-h-[72px] border-border-muted"
              >
                <div class="flex flex-col gap-1">
                  ...
                  <p
                    v-if="profile.bio"
                    class="text-[15px] text-text-muted line-clamp-1"
                  >
                    {{ profile.bio }}
                  </p>
                </div>
            ...
            <template
              v-if="!searchProfiles.isError.value"
              key="templateNoError"
            >
              <EmptyState
                key="noPageAndHasResultsState"
                v-if="
                  !searchProfiles.hasNextPage.value && profileResults.length > 0
                "
                title="You're all caught up!"
              ...
              <button
                key="hasNextPage"
                v-else-if="searchProfiles.hasNextPage.value"
                @click="handleLoadMoreProfile"
                :disabled="
                  !searchProfiles.hasNextPage.value ||
                  searchProfiles.isFetchingNextPage.value
                "
                class="px-6 py-3 mx-4 mt-4 font-medium rounded-xl border transition-colors border-border-muted bg-surface-app text-text-default hover:bg-surface-elevated disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {{
...
import type {
  GetFeedWithCursorResponse,
  ToggleFavoriteParams,
  ToggleLikeParams,
} from "../types";
import RefetchLoading from "../../../shared/components/loaders/RefetchLoading.vue";
...
const searchFeedTop = useSearchFeed(
  searchQuery,
  computed(() => "top"),
  {
    enabled: computed(() => activeFilter.value === "top"),
  }
);
const searchFeedRecent = useSearchFeed(
  searchQuery,
  computed(() => "recent"),
  {
    enabled: computed(() => activeFilter.value === "recent"),
  }
);

const activeSearchFeed = computed(() =>
  activeFilter.value === "top" ? searchFeedTop : searchFeedRecent
);

const feedResults = computed(() => {
  if (!activeSearchFeed.value.data.value?.pages) return [];
  return activeSearchFeed.value.data.value.pages.flatMap(
    (page: GetFeedWithCursorResponse) => page.data
  );
});
...
const handleTabClick = (tab: (typeof tabs)[number]) => {
  if (activeFilter.value === tab.id) {
    if (tab.id === "profile") {
      searchProfiles.refetch();
    } else {
      activeSearchFeed.value.refetch();
    }
    return;
  }
  setFilter(tab.id);
...
const handleLoadMoreFeed = () => {
  const feed = activeSearchFeed.value;
  if (feed.isFetchingNextPage.value || !feed.hasNextPage.value) return;
  feed.fetchNextPage();
};
const handleTryAgainFeed = () => {
  const feed = activeSearchFeed.value;
  if (feed.isRefetching.value) return;
  feed.refetch();
};
