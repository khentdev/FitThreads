<template>
  <ProfileViewLayout title="Profile">
    <div
      ...
      v-if="
        !profileStore.profileQuery.isLoading &&
        profileStore.profileQuery.isRefetching
      "
    >
      <Loader2 class="animate-spin size-5 stroke-border-strong" />
    </div>
    <ErrorRetry
      :retry-fn="profileStore.retryFetchProfile"
      v-else-if="errors.serverError || errors.profileFetchFailed"
    />
    <div
      v-else-if="profileStore.profileQuery.isPending"
      class="flex justify-center items-center absolute inset-0"
    >
      <Loader2 class="animate-spin size-5 stroke-border-strong" />
    </div>
    <template v-else>
      <ProfileHeaderOwn
        v-if="isOwnProfile"
        :profile="profileStore.profileQuery.data"
      />
...
import { useUserFavorites, useUserPosts } from "../../feed/composables";
...
const thoughtsQuery = useUserPosts(username);
const favoritesQuery = useUserFavorites(username);

const handleRefetch = () => {
  if (route.name === "profile-thoughts") thoughtsQuery.refetch();
  else if (route.name === "profile-favorites") favoritesQuery.refetch();
};

watch(
  () => route.params["username"],
  (newUsername) => {
    if (newUsername) profileStore.setViewedProfile(newUsername as string);
  },
  { immediate: true }
);
 ...
 */
watch(
  [() => errors.value.profileNotFound, () => authStore.hasAuthenticated],
  ([err, authenticated]) => {
    if (err) {
