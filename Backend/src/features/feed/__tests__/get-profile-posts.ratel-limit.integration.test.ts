...
import { rateLimitGetProfilePosts } from "../middlewares.js"
...
vi.mock("../../../configs/env.js", async (importOriginal) => {
    ...
    return {
        ...
        env: {
            ...actual.env,
            RATELIMIT_GET_PROFILE_POSTS_IP_MAX: 3,
            RATELIMIT_GET_PROFILE_POSTS_IP_WINDOW: 60,
        }
    }
})

describe("Get Profile Posts Rate Limit Integration Tests", () => {
    ...
    it("should allow requests within rate limit (3 req / 1 m)", async () => {
        ...
        for (let i = 0; i < 3; i++) {
            await rateLimitGetProfilePosts(ctx, next)
        }
    ...
    it("should reject requests exceeding rate limit (4th request)", { timeout: 10000 }, async () => {
        ...
        await expect(rateLimitGetProfilePosts(ctx, next)).rejects.toThrow(AppError)
    })
