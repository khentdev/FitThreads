...
describe("Send Magic Link Integration Tests", () => {
    const app = createApp()
    ...
    it("Should send magic link on verified user", async () => {
        ...
        expect(json.message).toEqual("Magic link sent - check your inbox.")

        const otpKeys = await redis.keys("otp:magic-link:*")
        expect(otpKeys.length).toBe(1)
        const otp = otpKeys[0].replace("otp:magic-link:", "")
