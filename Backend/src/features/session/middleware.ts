...
import { LogoutParamsController, SessionPayloadParams } from "./types.js"
...
export const validateLogoutSession = async (c: Context, next: Next) => {
    const isProd = env.NODE_ENV === "production"
    const sessionCookieName = isProd ? "__Secure-sid" : "sid"
    const sessionCookie = getCookieValue(c, sessionCookieName)
